...
func runMigrations(ctx context.Context, str *store.Store) error {
	...
	`); err != nil {
		return fmt.Errorf("failed to create login_history: %w", err)
	}

	// Crear índices
	str.Pool().Exec(ctx, `CREATE INDEX IF NOT EXISTS idx_login_history_user ON login_history(user_id)`)
	str.Pool().Exec(ctx, `CREATE INDEX IF NOT EXISTS idx_login_history_login_at ON login_history(login_at DESC)`)
	...
	`); err != nil {
		return fmt.Errorf("failed to create user_notification_settings: %w", err)
	}
